...
* @return string
*/
    public function json_deal($data){
        return json_decode(json_encode($data),true);
    }

    public function getCityInfo($lat,$long){
        $url = "http://api.map.baidu.com/geocoder/v2/?ak=IX1aUbuyiLkj9sMSz0sAMnFu8PH4u2rv&callback=renderReverse&location={$lat},{$long}&output=json&pois=1";
        $res = $this->getUrl($url);
        return $res;
    }

    public  function getUrl($url,$type='get',$data=array(),$timeout = 10){
        if($url == "" || $timeout <= 0){
            return false;
        }
